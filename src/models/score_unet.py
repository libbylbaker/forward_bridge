...
from flax.linen.linear import _conv_dimension_numbers, canonicalize_padding

# From https://github.com/yiyixuxu/denoising-diffusion-flax
...
class Unet(nn.Module):
    ...
    def __call__(self, x, time):
        ...
        for ind in range(num_resolutions):
            dim_in = h.shape[-1]
            h = ResnetBlock(
                dim=dim_in, groups=self.resnet_block_groups, dtype=self.dtype, name=f"down_{ind}.resblock_0"
            )(h, time_emb)
            hs.append(h)

            h = ResnetBlock(
                dim=dim_in, groups=self.resnet_block_groups, dtype=self.dtype, name=f"down_{ind}.resblock_1"
            )(h, time_emb)
        ...
        # middle
        h = ResnetBlock(dim=mid_dim, groups=self.resnet_block_groups, dtype=self.dtype, name="mid.resblock_0")(
            h, time_emb
        )
        h = AttnBlock(use_linear_attention=False, dtype=self.dtype, name="mid.attenblock_0")(h)
        h = ResnetBlock(dim=mid_dim, groups=self.resnet_block_groups, dtype=self.dtype, name="mid.resblock_1")(
            h, time_emb
        )
        ...
        out = ResnetBlock(dim=self.dim, groups=self.resnet_block_groups, dtype=self.dtype, name="final.resblock_0")(
            h, time_emb
        )
