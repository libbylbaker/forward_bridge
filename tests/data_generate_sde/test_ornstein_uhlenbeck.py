...
def test_correction_drift(times):
    corr = jnp.asarray([1.])
    t = 0.
    drift = ou.drift_correction(t, 0., corr)
    assert drift.ndim == 1
...
def test_backward(key, times):
    y = (1.,)
    score = partial(ou.score_forward, 0., -1.)
    drift, diffusion = ou.vector_fields()
...
def test_forward_score(key):
    ...
    y_true = true_score_fn(0., x0, t, x.flatten())
...
def test_important_reverse_and_correction_1d(key, times):
    ...
    x0 = jnp.asarray((1.0,))
    drift, diffusion = ou.vector_fields_reverse()
    sol = utils.important_reverse_and_correction(key, times, x0, y, drift, diffusion, ou.drift_correction).ys
