...
def main(key):
    ...
    y = data_boundary_pts.flattened_array_from_faces(fns)

    sigma = 0.04
    kappa = 1 / sigma

    sde_args = {"T": 1., "N": 100, "y": y, "num_landmarks": num_landmarks, "sigma": sigma, "kappa": kappa, "grid_size": 50, "grid_range": (-1.25, 1.25)}
    ...
    data_gen = sde_data.data_adjoint(jnp.zeros_like(y), kunita)
