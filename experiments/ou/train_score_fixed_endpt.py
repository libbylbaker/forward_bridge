...
data_fn = ou.data_reverse_distributed_y(sde["T"], sde["N"])
...
def main(key):
    ...
    for load in range(training["num_reloads"]):
        ...
        y_key = jr.split(data_key[0], 1)
        y = jr.uniform(y_key, shape=(training["load_size"], sde["dim"]), minval=0.0, maxval=6.0)
        ...
        for epoch in range(training["epochs_per_load"]):
            ...
            if actual_epoch % 20 == 0 or last_epoch:
                ...
                if last_epoch:
                    x0 = sde["x0"]
                    y = sde["y"]
                    plt.savefig(f"ou_endpt_x0_{x0}_y_{y}_score.pdf")
                plt.show()
                ...
                if last_epoch:
                    x0 = sde["x0"]
                    y = sde["y"]
                    plt.savefig(f"ou_endpt_x0_{x0}_y_{y}_traj.pdf")
                plt.show()
