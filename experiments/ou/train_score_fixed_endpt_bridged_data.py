...
data_fn = ou.data_reverse_importance(sde["x0"], sde["y"], sde["T"], sde["N"])
        ...
        for epoch in range(training["epochs_per_load"]):
            ...
            if actual_epoch % 20 == 0 or last_epoch:
                ...
                if last_epoch:
                    x0 = sde["x0"]
                    y = sde["y"]
                    plt.savefig(f"ou_endpt_x0_{x0}_y_{y}_score_bridge.pdf")
                plt.show()
                ...
                if last_epoch:
                    x0 = sde["x0"]
                    y = sde["y"]
                    plt.savefig(f"ou_endpt_x0_{x0}_y_{y}_traj_bridge.pdf")
                plt.show()
